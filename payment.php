...
<body>
    ...
    <div class="row">
        <div class="col-lg-6 mx-auto">
            <div id="container-payment-card" class="card ">
                    ...
                    <div class="tab-content">
                        <div class="row">
                                ...
                                if(isset($_POST['vehicle'])){
                                    ...
                                    if($vehicle == 'default'){
                                        $vehicle_proce = $price = $price + 0;
                                    }

                                    if($vehicle == 'car'){
                                        $vehicle_proce = $price = $price + 700;
                                    }
                                    if($vehicle == 'bike'){
                                        $vehicle_proce = $price = $price + 300;
                                    }
                                    if($vehicle == 'minivans'){
                                        $vehicle_proce = $price = $price + 500;
                                    }
                                    if($vehicle == 'electric_vehicle'){
                                        $vehicle_proce = $price = $price + 800;
                                    }
                                }
                           ...
                           if (mysqli_num_rows($result) > 0) {
                                ...
                                Payment Date: '.date("Y-m-d", strtotime('today')).'
                                </div>
                            <div class="col-lg-6">
                                ...
                                Time: '.$_POST['show'].'<br>
                                Booking Date: '.date("Y-m-d", strtotime('today')).'
                        ...
                        <div id="credit-card" class="tab-pane fade show active pt-3">
                            ...
                            $offer_sql = "select * from offer where userId = (SELECT id FROM user WHERE username = '$userna')";

                            $offer_results = mysqli_query($conn, $offer_sql);
                            $discount = 0;

                            while($row = mysqli_fetch_assoc($offer_results)){
                                if($row['collected'] == 1){
                                    $discount = $discount + $row['offerDiscount'];
                                    $offerId = $row['offerId'];
                                    $offer_sql_done = "update offer set collected = -1 where offerId = $offerId";
                                    mysqli_query($conn, $offer_sql_done);
                                }
                            }
                            $discount_price = ($price * $discount)/100;
                            $price = $price - $discount_price;
                            ?>
                                <div id="payable-container" style="margin-bottum: 20px; display: flex; flex-direction:column; align-items:center;justify-content:center;">
                                    <label style="border: 1px solid black; padding: 10px; margin-bottum: 50px; background-color: #000; color: #fff;">Amount Payble: Rs.<?php echo $price . "/- ( With parking: " . $vehicle_proce . "/-) (Discount: Rs." . $discount_price . "/-)"?></label>
                                </div>
                                <div id="msg"></div>
